...
'''class Post_data(APIView):
    def get(self,request,pk):
        id=pk
        usr = Post.objects.filter(user=CustomUser.objects.get(pk=pk))
        serializer = User_Post_serializer(usr,many = True)
        return Response(serializer.data)'''
...
class Post_data(APIView):
    ...
    permission_classes = [IsAuthenticated]
    def get(self,request,pk):
        usr = Post.objects.filter(user=Post.objects.get(pk=pk))
        serializer = User_Post_serializer(usr,many = True)
        return Response(serializer.data)              
